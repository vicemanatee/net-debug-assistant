...
void controller::outputClientMessage()
{
...
//not adding head
    {
        dialogHistory.append("[" + QTime::currentTime().toString(timeFormat) + "] "
                             + tr("Client:")+ SCmodel->clientMessage.toHex() + "\n");
        if(ui->RBreceiveSetAsc->isChecked())
        //appear ASC
            ui->Edialog->append("[" + QTime::currentTime().toString(timeFormat) + "] " + tr("Client:") + SCmodel->clientMessage);
        else
        //apear HEX
            ui->Edialog->append("[" + QTime::currentTime().toString(timeFormat) + "] " + tr("Client:") + SCmodel->clientMessage.toHex());
    }
    else{
//adding head
        if (QString(SCmodel->clientMessage).startsWith("$$$$^")){
    //cutting head
            processedMsg = QString(SCmodel->clientMessage).right(
                        QString(SCmodel->clientMessage).length()-head.length());
...
void controller::on_BsendSetBreak_clicked()
{
    ...
    ui->CBcastSelect->setEnabled(true);
...
void controller::on_CBtypePro_currentIndexChanged(const QString &arg1)
{
    ...
    {
        ui->CBcastSelect->setEnabled(false);
        ui->BactivateListen->setEnabled(true);
...
void controller::repeatDialog(QString side, QString content)
{
    ...
    if(side == " Server" && SCmodel->m_tcp->state() == QAbstractSocket::ConnectedState)
    {//send from server
        if (ui->CBsendAddHead->isChecked() == false)
        {
        //no head
